...
flags.DEFINE_string('atr_path', '../ATR/humanparsing/JPEGImages', 'Image to run')
flags.DEFINE_string(
    'json_path', None,
...
def main(dir_path, json_path=None):
    if not os.path.exists('./3DATR_hmr_mask'):
        os.mkdir('./3DATR_hmr_mask')
    sess = tf.Session()
    ...
    for root, dirs, files in os.walk(dir_path, topdown=True):
            for img_path in files:
                ...
                w, h, _ = img.shape
                img = cv2.resize(img, dsize=(h//4, w//4), interpolation=cv2.INTER_NEAREST)
...
def save_mesh(img, img_path, split, proc_param, joints, verts, cam, faces):
    ...
    img_copy = img.copy()
    mask_name = './3DATR_hmr_mask/%s.png'%(os.path.basename(img_path) )
...
if __name__ == '__main__':
    ...
    main(config.atr_path, config.json_path)
